...
/*
 * This file contains simple functions to deal with winbond spi flash.
 * Two implementations are possible: Using the xilifs library or using the SPI directly.
 *
 *
 */
...
/******* simple API ******/
/*
 * These functions are blocking - they do not return until the operation is completed.
 * Reads/writes are done in 'QUAD' mode.
 * Return values are either XST_SUCCESS or XST_FAILURE.
 */

int flash_init(void);
int flash_complete_erase(void);
int flash2ram(u32 flash_address, int length, void *ram_dst);
int ram2flash(void *ram_src, int length, u32 flash_address);




#define USE_LIBXILIFS

#ifdef USE_LIBXILIFS
/********************************* XILIFS IMPLEMENTATION *************/
...
#define INTC_DEVICE_ID_F			XPAR_INTC_0_DEVICE_ID
...
static int SetupInterruptSystem(XSpi *SpiPtr)
{
	 ...
	 * xparameters.h
	 */
	Status = XIntc_Initialize(&InterruptController, INTC_DEVICE_ID_F);
	if(Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
...
int flash2ram(u32 flash_address, int length, void *ram_dst)
{
	char * data = (char *)ram_dst;
	int Status;
	XIsf_ReadParam ReadParam;
	u32 Index;
	u32 Address = flash_address;
	...
	while (length > 0) {
		...
		for(Index = 0; Index < ISF_PAGE_SIZE; Index++) {
			if (Index < length)
				data[Index] = ReadBuffer[Index + XISF_CMD_SEND_EXTRA_BYTES +  QUAD_READ_DUMMY_BYTES];
			else
				break;
		}

		// onto next page
		length -= ISF_PAGE_SIZE;
		data += ISF_PAGE_SIZE;
...
int ram2flash(void *ram_src, int length, u32 flash_address)
{
	...
	while (length > 0)
	{
		...
		if (length >= ISF_PAGE_SIZE) {
			for(Index = 0; Index < ISF_PAGE_SIZE; Index++) {
				//WriteParam.WritePtr[Index] = Index + 0x10;
				WriteParam.WritePtr[Index] = data[Index];
			}
		} else {
			for(Index = 0; Index < length; Index++) {
				...
				if (Index < length)
					...
					WriteParam.WritePtr[Index] = 0xff;
			}
		}
		...
		// onto next page
		length -= ISF_PAGE_SIZE;
		Address += ISF_PAGE_SIZE; // increase flash address
		data += ISF_PAGE_SIZE; // increase pointer
	}
